    ...
    <div v-if="items.length" class="flex flex-col lg:flex-row lg:items-start gap-8">
      ...
      <div class="flex-1">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-6 gap-4">
          ...
          <div v-if="totalPages > 1" class="flex justify-center w-full sm:w-auto">
            <el-pagination
      ...
      <!-- Информация о заказе -->
      <div class="w-full lg:w-80 bg-gray-50 border rounded-xl p-6 shadow-md space-y-4 self-start sticky top-24">
        <h3 class="text-lg font-semibold text-gray-800">Ваша корзина</h3>
        <div class="flex justify-between text-sm text-gray-600">
          <span>Товары</span>
          <span>{{ totalCount }} шт.</span>
        </div>
        <div class="flex justify-between items-center font-semibold text-base border-t pt-4 text-gray-800">
          <span>Итого</span>
          <span class="text-xl text-primary-600 font-bold">{{ totalPrice.toFixed(2) }} ₽</span>
        </div>
          <!-- Кнопка оформления -->
          <el-button
            type="primary"
