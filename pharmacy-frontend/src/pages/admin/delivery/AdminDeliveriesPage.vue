    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
      <el-form :inline="true" @submit.prevent>
        <el-form-item label-width="0">
          <el-input v-model="filters.orderNumber" placeholder="Номер заказа" size="large" class="!w-40" />
        </el-form-item>
      ...
      <table class="min-w-full table-fixed divide-y divide-gray-200 text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr v-if="!loading && deliveries.length === 0">
            <td colspan="5" class="text-center py-6 text-gray-500">Доставки не найдены</td>
          </tr>
          <tr v-if="loading">
...
const fetch = async () => {
  ...
  try {
    ...
    const data = await getDeliveries({ page: pageNumber.value, size: pageSize.value, filters: payload })
