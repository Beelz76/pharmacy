<template>
  <div
    class="flex items-center w-full border border-gray-300 bg-white rounded-md relative"
  >
    <el-popover
...
const fetchSearchSuggestions = async (query) => {
  ...
  try {
    const response = await api.get(
      `/products/search-suggestions?query=${encodeURIComponent(query)}`
    );
